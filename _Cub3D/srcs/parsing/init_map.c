...
static int	ft_value_pos(t_hold *hold, t_pt pos)
{
	hold->pos_x = pos.x;
	hold->pos_y = pos.y;
...
int	ft_init_map_pt(t_hold *hold)
{
	...
	while (hold->map[++pos.y])
	{
		...
		{
			if (!hold->pos_x && ft_is_valid_player_dir(hold->map[pos.y][pos.x]))
			{
				if (ft_value_pos(hold, ft_point(pos.x, pos.y)) == ERR)
					return (ERR);
			}
			else if (hold->pos_x
				&& (ft_is_valid_player_dir(hold->map[pos.y][pos.x])))
